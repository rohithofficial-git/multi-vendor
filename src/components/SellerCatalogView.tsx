...
export default function SellerCatalogView({ sellerId }: SellerCatalogViewProps) {
  ...
  const filteredProducts = useMemo(() => {
    // Start with all products, then filter by active seller
    let result = products.filter(p => p.seller_id === sellerId);
    ...
    return result;
  }, [products, sellerId, searchVal, selectedCategory, maxPrice, minRating, inStockOnly, sortBy]);

  return (
    <div className="w-full">
      {/* Catalog Control Header */}
      <div className="flex flex-col md:flex-row md:items-center md:justify-between border-b border-theme-border/50 pb-6 mb-8 gap-4">
        <div>
          <h2 className="font-display text-2xl font-bold text-theme-text">Your Studio Collection</h2>
          <p className="text-xs text-theme-muted mt-1">Preview how your published items display to potential buyers.</p>
        </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full rounded-xl border border-theme-border bg-theme-bg-from/50 py-2.5 pl-9 pr-3 text-xs outline-none focus:border-brand text-theme-text placeholder-theme-muted"
                />
        ...
        <section className="md:col-span-3">
          ...
          {filteredProducts.length > 0 ? (
            ...
            <div className="glass-panel rounded-3xl p-12 text-center border border-theme-border flex flex-col items-center justify-center">
              ...
              <h3 className="font-display text-lg font-bold text-theme-text">No designs found</h3>
              <p className="text-xs text-theme-muted mt-2 max-w-sm">
                Try tweaking your search terms, categories, or slide values.
              </p>
              <button
                onClick={handleResetFilters}
              ...
              <div className="flex-grow overflow-y-auto py-4 space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    value={searchVal}
                    onChange={(e) => setSearchVal(e.target.value)}
                    className="w-full rounded-xl border border-theme-border bg-theme-card/30 py-2 px-3 text-xs outline-none text-theme-text"
                  />
                </div>
